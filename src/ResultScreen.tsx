...
export default function ResultScreen({
  ...
  // No cachedComprehensiveReport needed — PDF uses displayData only, no secondary Gemini call.
  ...
  // No background PDF generation — PDF is generated instantly on-device when user taps Download.
...
Generated by MedPlant App
  ...
  // Generate PDF on-device using expo-print (instant, no network)
  const handleDownloadPDF = async () => {
    // WEB: Use existing HTML download flow
    if (Platform.OS === 'web') {
    ...
    // MOBILE: Generate PDF locally on-device (instant)
    if (isGeneratingPDF) return;
    setIsGeneratingPDF(true);
    setLoadingStatus('Generating PDF...');

    try {
      const htmlContent = generateHtml({ plant, therapeuticProfile: detailedInfo });

      // Generate PDF on-device — no network call, instant
      const { uri } = await Print.printToFileAsync({ html: htmlContent });
      console.log('[PDF] Generated locally at:', uri);

      // Open share sheet so user can save/send the PDF
      ...
      console.error('[PDF] Local generation error:', error);
      Alert.alert('PDF Failed', error.message || 'Could not generate PDF.');
    } finally {
  ...
  // WEB ONLY: Build HTML from stored displayData (no Gemini call)
  const startPDFGeneration = async () => {
    console.log('[PDF] Web HTML Generation from stored displayData...');
    try {
      setIsGeneratingPDF(true);
      setLoadingStatus('Preparing report from scan data...');

      // Use existing displayData — formatting only, zero AI calls
      const htmlContent = generateHtml({ plant, therapeuticProfile: detailedInfo });
  ...
  // handleOpenPdf removed — PDF is generated and shared instantly on download tap.
