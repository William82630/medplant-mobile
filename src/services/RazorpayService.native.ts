...
const initiatePayment = async (
  ...
  try {
    ...
    if (result.type === 'success' && result.url) {
      ...
      // UPI / Google Pay opens a separate app, which destroys the Custom Tab.
      // The browser returns "dismiss" even though the payment may have succeeded.
      // Poll the backend to check the Razorpay order status (UPI can take time).
      console.log('[RazorpayWeb] Browser dismissed — polling verify-payment...');

      const MAX_RETRIES = 5;
      const POLL_INTERVAL_MS = 3000; // 3 seconds between each attempt

      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          await new Promise(resolve => setTimeout(resolve, POLL_INTERVAL_MS));

          const verifyUrl = `${BACKEND_URL}/api/verify-payment?order_id=${encodeURIComponent(order_id)}`;
          console.log(`[RazorpayWeb] verify-payment attempt ${attempt}/${MAX_RETRIES}...`);
          const verifyRes = await fetch(verifyUrl);
          const verifyJson = await verifyRes.json();
          console.log(`[RazorpayWeb] verify-payment response:`, JSON.stringify(verifyJson));

          if (verifyJson.paid) {
            console.log('[RazorpayWeb] ✅ Payment confirmed + activated via verify-payment!');
            console.log('[RazorpayWeb] activated:', verifyJson.activated, 'credits:', verifyJson.credits);
            onSuccess({ razorpay_order_id: order_id });
            return;
          }

          console.log(`[RazorpayWeb] Not yet paid (status: ${verifyJson.status}). Retrying...`);
        } catch (verifyErr: any) {
          console.warn(`[RazorpayWeb] verify attempt ${attempt} failed:`, verifyErr?.message);
        }
      }

      // Only report cancelled after ALL retries exhausted
      console.log('[RazorpayWeb] Payment not confirmed after all retries — treating as cancelled');
      onError('Payment cancelled');
    } else {
      onError('Payment cancelled');
    }
