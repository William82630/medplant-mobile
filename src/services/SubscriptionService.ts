...
// Hardcoded admin emails for immediate access restoration
const ADMIN_EMAILS = ['willsblogger82@gmail.com'];

/**
 * Helper to check if an email belongs to a hardcoded admin
 */
...
export async function getOrCreateSubscription(userId: string): Promise<UserSubscription | null> {
  try {
    // 0. CHECK FOR HARDCODED ADMIN OVERRIDE (Explicit & Centralized)
    // We fetch the user to get the email since userId doesn't have it
    const { data: { user } } = await supabase.auth.getUser();

    // Only apply override if the requested userId matches the authenticated user
    // (Security check to ensure we don't expose admin status incorrectly)
    if (user && user.id === userId && isHardcodedAdmin(user.email)) {
      // console.log('[Admin: Sub] Hardcoded admin detected:', user.email);
      return {
        id: 'admin-override',
        user_id: userId,
        plan: 'pro_unlimited',
        is_pro: true,             // Always Pro
        daily_credits: 99999,     // Unlimited credits visual
        last_reset_date: new Date().toISOString().substring(0, 10),
        subscription_id: 'admin-bypass',
        plan_start_date: new Date().toISOString(),
        plan_end_date: new Date(Date.now() + 1000 * 60 * 60 * 24 * 365 * 10).toISOString(), // 10 years
        is_admin: true,           // Explicit Admin Flag
    ...
    // Actual server or network errors should still be logged
    if (fetchError) {
    ...
    // If no subscription row exists, treat as a non-pro user
    return {
      id: `fallback-${userId}`,
      user_id: userId,
      plan: 'free',
  ...
  try {
    ...
    const { data: updated, error } = await supabase
      ...
      .upsert({
        ...
        last_reset_date: now.toISOString().substring(0, 10),
