...
export default function ResultScreen({
  ...
  const [isPreparingPdf, setIsPreparingPdf] = useState(false); // Mobile spinner state
...
Generated by MedPlant App
  ...
  const handleDownloadPDF = async () => {
    // WEB: Use existing flow with loading overlay
    if (Platform.OS === 'web') {
    ...
    // MOBILE: Just use the same instant text share (same as Share PDF button)
    handleShareReport();
  };

  // MOBILE ONLY: Direct PDF share with inline spinner
  const sharePDF = async () => {
    // Credit check
    if (!hasCredits() && !isAdmin()) {
      Alert.alert("Out of Credits", "You need credits to download the premium report.");
      return;
    }

    // Show spinner immediately (visual only)
    setIsPreparingPdf(true);

    try {
      // Generate report data
      const reportData = await generateComprehensiveReport(plant.commonName || plant.scientificName || 'Unknown Plant');
      ...
      // Build text for PDF
      const plantData = reportData?.plant || plant;
      const reportText = `
REPORT: ${plantData.commonName || 'Unknown Plant'}
Scientific Name: ${plantData.scientificName || 'N/A'}
Family: ${plantData.family || 'N/A'}

${reportData?.therapeuticProfile || detailedInfo || 'Not available'}

Generated by MedPlant AI
      `.trim();

      // Generate PDF file
      const pdfUri = await downloadPdfReport(plantData.commonName || 'Plant', reportText);

      // SHARE IMMEDIATELY
      await Sharing.shareAsync(pdfUri, {
        mimeType: 'application/pdf',
        dialogTitle: `${plantData.commonName || 'Plant'} Report`,
        UTI: 'com.adobe.pdf',
      });

    } catch (error: any) {
      Alert.alert('PDF Failed', error.message || 'Could not generate report');
    } finally {
      // Hide spinner after share sheet closes
      setIsPreparingPdf(false);
    }
  };

  const startPDFGeneration = async () => {
    // CREDIT LOGIC FIX (FINAL): 
    // Credit is paid ONCE during plant identification in ProAIScanScreen.
    // Downloading the comprehensive report is FREE - no credit check or deduction here.
    // Caching is used for performance (to avoid redundant AI API calls).
    const hasCachedReport = cachedComprehensiveReport.current !== null;
    ...
    try {
      ...
      if (hasCachedReport) {
        // USE CACHED REPORT - No API call needed
        console.log('[PDF] Using cached comprehensive report');
        setLoadingStatus('Preparing cached report...');
        comprehensiveData = cachedComprehensiveReport.current;
      } else {
        // GENERATE NEW COMPREHENSIVE REPORT - NO credit needed (already paid during identification)
        setLoadingStatus('Generating comprehensive analysis...');
        console.log('[PDF] Generating comprehensive report (no credit deduction - paid during identification)');
        comprehensiveData = await generateComprehensiveReport(plant.commonName || plant.scientificName || 'Unknown Plant');

        // Cache the report for future downloads/shares
        if (comprehensiveData) {
          cachedComprehensiveReport.current = comprehensiveData;
          console.log('[PDF] Report cached for future use');
        }
        // NO credit deduction here - credit was already paid in ProAIScanScreen
      }
...
Generated by MedPlant AI
      ...
      if (isWeb) {
        // WEB: Download HTML file directly with improved reliability
        ...
        // Append to document body (required for Firefox)
        ...
        // MOBILE: Call Backend for PDF and immediately open share sheet
        const pdfUri = await downloadPdfReport(cPlant.commonName || 'Plant', reportText);
        console.log('[PDF] Downloaded to:', pdfUri);

        // Immediately open share sheet (same as Share Report behavior)
        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(pdfUri, {
            mimeType: 'application/pdf',
            dialogTitle: `${cPlant.commonName || 'Plant'} Report`,
            UTI: 'com.adobe.pdf',
          });
        }

        // Save URI for future reference
      ...
      const errorMessage = `Could not generate report: ${error.message}`;
      if (Platform.OS === 'web') {
        window.alert("PDF Failed\n\n" + errorMessage);
      } else {
        Alert.alert('PDF Failed', errorMessage);
      }
    } finally {
    ...
    <View style={[styles.main, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Pressable
          ...
          disabled={Platform.OS !== 'web' && isPreparingPdf}
          style={({ pressed }) => [
            styles.downloadButton,
            {
              opacity: (Platform.OS === 'ios' && pressed) || isPreparingPdf ? 0.7 : 1
            }
          ]}
          android_ripple={{ color: '#4CAF5030', borderless: false }}
        >
          {Platform.OS !== 'web' && isPreparingPdf ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
              <ActivityIndicator size="small" color="#fff" style={{ marginRight: 10 }} />
              <Text style={styles.downloadButtonText}>Preparing report… please wait</Text>
            </View>
          ) : (
